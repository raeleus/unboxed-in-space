...
public class ShootAsteriskBehaviour extends BehaviourAdapter {
    protected float delay = .35f;
    protected float bulletVelocity = 7f;
    protected float damage = 25f;
    private float timer;

    public ShootAsteriskBehaviour(GameObject gameObject) {
        super(gameObject);
    }
    ...
    public void update(float delta) {
        ...
        if (timer < 0) {
            ...
            go = new GameObject(unBox);
            velocity = new Vector2(bulletVelocity, 0);
            velocity.rotateDeg(MathUtils.radDeg * body.getTransform().getRotation() + 270);
            new ProjectileBehaviour(go, velocity, damage);
            ...
            go = new GameObject(unBox);
            velocity = new Vector2(bulletVelocity, 0);
            velocity.rotateDeg(MathUtils.radDeg * body.getTransform().getRotation() + 90);
            new ProjectileBehaviour(go, velocity, damage);
