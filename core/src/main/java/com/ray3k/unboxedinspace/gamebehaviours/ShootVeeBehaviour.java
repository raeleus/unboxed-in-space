...
public class ShootVeeBehaviour extends BehaviourAdapter {
    protected float delay = .1f;
    protected float bulletVelocity = 10f;
    protected float damage = 25f;
    private float timer;

    public ShootVeeBehaviour(GameObject gameObject) {
        super(gameObject);
    }
    ...
    public void update(float delta) {
        ...
        if (timer < 0) {
            ...
            velocity.rotateDeg(MathUtils.radDeg * body.getTransform().getRotation() - 30);
